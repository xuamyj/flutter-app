...
export default class RoundButtonSmall extends React.Component {
  ...
  render() {
    if (this.props.isActive === true) {
      return (
        <TouchableOpacity style={[styles.buttonContainer, this.props.containerStyle, {backgroundColor: this.props.backgroundColor}]} onPress={this.props.onPress}>
      ...
      return (
        <View style={[styles.buttonContainer, this.props.containerStyle, {backgroundColor: this.props.backgroundColor}]}>
          <Text style={{letterSpacing: 1, fontWeight:"bold", color: this.props.color, fontSize: this.props.size, fontFamily: 'NunitoBold'}}>{this.props.label}</Text>
        </View>
      );
    }
